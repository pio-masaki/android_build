...
page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/INFAMOUS-LOLLIPOP/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for INFAMOUS-LOLLIPOP/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: INFAMOUS-LOLLIPOP/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "INFAMOUS-LOLLIPOP/%s" % repo_name, "revision": "lollipop" })
...
print "Repository for %s not found in the INFAMOUS Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/infamous_manifest.xml" % device
